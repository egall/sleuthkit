...
static int open_filename_with_suffix(string &filename)
{
    for (int i = 0; i < 10000; i++)
    {
        ...
        {			      // otherwise, try a numeric suffix
        ...
        if (fd > 0)
        {			// open is successful; return the fd
...
void content::open_tempfile()
{
    ...
    for (string::const_iterator cc = evidence_filename.begin(); cc != evidence_filename.end(); cc++)
    {
        if (isalnum(*cc) || *cc == '.')
        {
    ...
    if (fd_temp < 0)
        errx(1, "cannot open temp file %s:", tempfile_path.c_str());
}
...
void content::write_record(byteRunsFacet facet )
{
    if (opt_magic)
    {
    ...
    if (this->segs.size() > 0)
    {
        ...
        { // testing
            ...
            {
                sprintf(buf,
                        "       <byte_run file_offset='%"PRIu64"' fill='0' len='%"PRIu64"'/>\n",
                        i->file_offset, i->len);
            }
            ...
            else if (i->flags & TSK_FS_BLOCK_FLAG_COMP)
            {
                if (i->fs_offset)
                {
                    sprintf(buf, "       <byte_run file_offset='%"PRIu64"' fs_offset='%"PRIu64"' "
                    "img_offset='%"PRIu64"' uncompressed_len='%"PRIu64"'/>\n", i->file_offset,
                            i->fs_offset, i->img_offset, i->len);
                }
                else
                {
                    sprintf(buf,
        ...
        switch (facet) {
            case DATA:
                file_info_xml("byte_runs","facet=\"data\"", runs);
                break;
            case INODE:
                file_info_xml("byte_runs","facet=\"inode\"", runs);
                break;
            case NAME:
        ...
        if (!invalid)
        {
            if (opt_md5 && h_md5.hashed_bytes > 0)
                file_info(h_md5.final());
            if (opt_sha1 && h_sha1.hashed_bytes > 0)
                file_info(h_sha1.final());
        }
    }
...
{
    return opt_md5 || opt_sha1 || opt_save || do_plugin || opt_magic || opt_get_fragments
            || opt_body_file;
//      || opt_compute_sector_hashes;
}

/** Called to create a new segment. */
void content::add_seg(int64_t img_offset, int64_t fs_offset, int64_t file_offset, int64_t len,
...
void content::add_bytes(const u_char *buf, uint64_t file_offset, ssize_t size)
{
    ...
    {
        if (opt_md5)
            h_md5.update((unsigned char *) buf, size);
        if (opt_sha1)
            h_sha1.update((unsigned char *) buf, size);
    }
    if (fd_save)
    {
        ...
        if (fd_save)
        {
            ...
            if (res != size)
            {
                warn("write(%d,%p,%"PRId64")=%"PRId64, fd_save, buf, (int64_t) size, (int64_t) res);
                close(fd_save);
                fd_save = 0;
            }
        }
    ...
    if (fd_temp)
    {
        ...
        if (fd_temp)
        {
            ...
            if (res != size)
            {
                warn("write(%d,%p,%"PRId64")=%"PRId64, fd_temp, buf, (int64_t) size, (int64_t) res);
                close(fd_temp);
                fd_temp = 0;
            }
        }
...
{
    if (fd_save)
    {			// close the save file if it exists
        close(fd_save);
        if (total_bytes == 0)
        {		// unlink the save file if it has no bytes
    ...
    if (tempfile_path.size() > 0)
    {		// unlink temp file if one was created
