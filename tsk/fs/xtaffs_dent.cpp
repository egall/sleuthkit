...
/**
 * \file xtaffs_dent.cpp
 * Contains the internal TSK XTAF file name processing code.
 */
...
*/
static uint8_t
    xtaffs_dir_buf_get(XTAFFS_INFO * xtaffs, TSK_INUM_T dir_inum,
    TSK_INUM_T *par_inum)
{
    uint8_t retval = 1;
    tsk_take_lock(&xtaffs->dir_lock);
    ...
    TSK_INUM_T a_addr)
{
    ...
    if ((a_addr < a_fs->first_inum) || (a_addr > a_fs->last_inum)) {
        ...
        tsk_error_set_errstr("xtaffs_dir_open_meta: invalid a_addr value: %"
            PRIuINUM "\n", a_addr);
        return TSK_ERR;
    }
    ...
    if (fs_dir->fs_file == NULL) {
        ...
        tsk_error_set_errstr("xtaffs_dir_open_meta: %" PRIuINUM
    ...
    */
    ...
    if (tsk_fs_file_walk(fs_dir->fs_file,
        ...
        xtaffs_dent_action, (void *) &load)) {
            tsk_error_errstr2_concat("- xtaffs_dir_open_meta");
            free(dirbuf);
            free(addrbuf);
            return TSK_COR;
    }
